...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Nusantara Vpn ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Author : ${green}Nusantara® ${NC}${YELLOW}(${NC} ${green} Nusantara Vpn ${NC}${YELLOW})${NC}"
echo -e " © Recode By Nusantara®${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
if [[ $( uname -m | awk '{print $1}' ) == "x86_64" ]]; then
    echo -e "${OK} Os Kamu Di Support ( ${green}$( uname -m )${NC} )"
else
    echo -e "${EROR} Os Kamu Tidak Di Support! ( ${YELLOW}$( uname -m )${NC} )"
    exit 1
fi
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/joytun21/schaya/main/izin | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/joytun21/schaya/main/izin | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/joytun21/schaya/main/izin | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
function is_root() {
    if [[ 0 == "$UID" ]]; then
        print_ok "Root User Installasi Prosess"
    else
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/joytun21/schaya/main/izin | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/joytun21/schaya/main/izin | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>⚡AUTOSCRIPT PREMIUM⚡</b>
<b>⚡BY NUSANTARA TUNNEL⚡</b>
<code>────────────────────</code>
...
"'&reply_markup={"inline_keyboard":[[{"text":"Order","url":"https://t.me/tetet"},{"text":"ORDER","url":"https://wa.me/6285782928729"}]]}'
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    print_install "Memasang Konfigurasi Packet Nginix Server"
...
wget raw.githubusercontent.com/Felixind/VVIPAutoScript/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
echo -e "${green} AutoScript By Nusantara Tunneling Berhasil Dipasang✓"
echo ""
